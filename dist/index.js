...
const resolveTasks = () => {
    ...
    const totalWords = (0, utils_1.getNumberOfWords)(TEXT);
    console.log(`A: ${totalWords} words`);
    // Q2
    console.log('Q: How many sentences are there in the text?');
    console.log(`A: ${(0, utils_1.getNumberOfSentences)(TEXT)} sentences`);
    // Q3
    console.log('Q: What is the length of the longest word?');
    console.log(`A: ${(0, utils_1.getLongestWordLength)(TEXT)} characters`);
    // Q4
    console.log('Q: Which six words occur the most in the text?');
    const allWords = (0, utils_1.getAllWords)(TEXT);
    const allWordsFrequency = (0, utils_1.getWordsFrequency)(allWords);
    const words = (0, utils_1.getWordsWithHighestFrequency)(6, allWordsFrequency);
    console.log(`A: ${words}`);
    // Q5
    console.log('Q: What percentage of the words only occur once?');
    const { groupedMap } = (0, utils_1.groupWordsByFrequency)(allWordsFrequency);
    const percentage = (groupedMap['1'].length / (0, utils_1.getNumberOfWords)(TEXT)) * 100;
    ...
    console.log(`A: ${(0, utils_1.getNumberOfWords)(TEXT) / (0, utils_1.getNumberOfSentences)(TEXT)} words`);
    // Q7
    console.log('Q: Which three two-word phrases occur the most in the text?');
    const wordsGroupedByTwo = (0, utils_1.splitTextInGroupsOfTwo)(TEXT);
    const wordsGroupedByTwoFrequencyMap = (0, utils_1.getWordsFrequency)(wordsGroupedByTwo);
    const wordsGroupedByTwoMostFrequent = (0, utils_1.getWordsWithHighestFrequency)(3, wordsGroupedByTwoFrequencyMap);
    ...
    const fiveMostFrequentWords = (0, utils_1.getWordsWithHighestFrequency)(5, allWordsFrequency);
    console.log('A:');
    fiveMostFrequentWords.forEach(word => {
        const positionSum = (0, utils_1.getPositionSumOfWord)(allWords, word);
